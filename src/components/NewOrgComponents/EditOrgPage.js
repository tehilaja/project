import React from 'react';

import { Redirect } from "react-router-dom";

import { v1 as uuid } from 'uuid';


import axios from "axios";

import { async } from "q";
import ImageUploader from 'react-images-upload';
import { GoOrganization } from "react-icons/ai";
////---------------

import Header from '../Header.js';
import NewOrg from './NewOrg.js';
import Footer from '../Footer.js';
import AddPrizes from './AddPrizes.js';

import {Button,Input,Container,Divider,Form,Grid,Icon,Image,Label,List,Menu,Radio,Responsive,Segment,Sidebar,Step,Visibility,} from 'semantic-ui-react'

const uploadUtil = require('../../utilities/upload').methods;
const emailService = require('../../utilities/email');

class EditOrgPage extends React.Component {

    constructor(props) {
        super(props)
        this.state = {
            orgId: this.props.id,
            loggedIn: this.props.data.loggedIn,
            program_admin: this.props.data.program_admin,            
            userName: this.props.data.userName,
            adminPhone: this.props.data.phone,
            //////////////////
            orgName: '',
            admin_name: '',
            description: '',
            field_of_activity: '',
            phone: '',
            email: '',
            //mailing information:
            country: "",
            city: "",
            building: "",
            street: "",
            p_code: "",
            //more info:
            pictures: [],
            minDonation: 10,
            allowOneTimeDonations: false,
            // org_num:"",
            // branch:"",
            // account_num:"",
            // bank_num:"",
            // description:"",
            // field_of_activity:"",
            // founding_year:"",
            // working:"",
            // volunteers:"",
            flag_done: false,
            file: "",
            file_name: "",
            selectedImage: null,
            image_url: null,

            routeMain: false,
            showAddPrize: false,

// org_id org_name admin_name description field_of_activity img_url min_donation approved org_num branch account_num bank_num account_owner, one_time_donation, founding_year working volunteers friends city_name ,country_name, building street p_code 

            // req json
            newOrg_req: {"org_id": 2, "org_name":'',"min_donation": '',"approved":0,"org_num":'',
                "branch":'', "account_num":'', "bank_num":'', "account_owner":'', "one_time_donation": 0,  // must field
                "admin_name": '',"img_url": '',
                "description": '',"field_of_activity": '',"founding_year":'', "working":'', "volunteers":'', "friends":'', "city_name":'', "building": '', "street":'', "p_code":''},
        }
        this.handleChange = this.handleChange.bind(this)
        this.handlerClick = this.handlerClick.bind(this);
        this.increment = this.increment.bind(this);
        this.decrement = this.decrement.bind(this);
        this.onSelectFile = this.onSelectFile.bind(this);
        //
        this.addOrgProcess = this.addOrgProcess.bind(this);
        this.sendEmail = this.sendEmail.bind(this);
        //this.state.org = this.fetchOrgData(this.state.orgId)
        this.handleSubmit = this.handleSubmit.bind(this);
        
        this.fetchOrgData();
    }

    //the following function is to send an email that a new oragnization is awaiting approval
    sendEmail() {
        // TODO: get text to send, get list of mail
        emailService.sendEmail(
            //'mordeyj316@gmail.com'
            ['tehilaj97@gmail.com'],
            null,
            null,
            'A new organization is awaiting your approval',
            'organization name: '+ this.state.orgName +
            '\norganizaion description: ' + this.state.description +
            '\nclick to view org image: '+ this.state.image_url+
            '\nname of user asking to create platform: ' + this.state.userName,
            null,
        )
    }
    
    
    // ~~~~~~~ add new org to DB
    addOrgProcess(){ 

    // first step
    axios.post('/addOrg/firstStep', this.state.newOrg_req
    ).then(res => 
    {
        // alert("res is: " + res.data)

    }).catch(error=> {
        alert("error addOrgProcess" +  error);
    })

    }


    //another try:
    fileSelctedHandler = event => {
        this.setState({
            pictures:event.target.files[0]
        })
        this.fileUploadHandler();
    }

    fileUploadHandler = () => {
        (async () => {
            const response = await axios.post(
                '/upload-file',
                {
                    file: this.state.pictures,
                    key: 'try/try.jpg',
                },
                {header:{'Content-Type': 'application/json'}}
                );
               console.log("resp",response);
             
          })();  
    }

    onSelectFile(event) {
        emailService.emailAttachementFromFile(event.target.files[0], (attachement) => this.setState({ selectedImage: attachement }));        
    }


    handleChange(event) {
        const { name, value, type, checked } = event.target
        type === "file" ? this.setState({ [name]: event.target.files[0] }) : this.setState({ [name]: value })
    }



    handleSubmit = (e) => {

        (async () => {
            const fileNamePrefix = `${this.state.orgName}_${uuid()}`;
            //TODO: save url to db in callback - save all org information to DB
            await uploadUtil.uploadDataUrlFile(this.state.selectedImage.path, this.state.selectedImage.filename, fileUrl => alert('file url:' + fileUrl), fileNamePrefix, 'organizations'); 
            // exp: => (callback, who, folder in s3)
        })();
        this.sendEmail();
        e.preventDefault();
    }

    //function to deal with passing state to parent component:
    handlerClick(flagDone) {
        this.setState({
            flagDone: true,
        });
        this.props.record(flagDone)
    }

    //---------increment + ----------

    increment() {
        this.setState(prevState => {
            return {
                minDonation: prevState.minDonation + 1
            }
        })
    }

    //---------decrement - -------------
    decrement() {
        if (this.state.minDonation > 1)
            this.setState({ minDonation: this.state.minDonation - 1 })
    };

    //TODO: get the information (orgId for the correct organization to Edit)
    fetchOrgData(){
        (async () => {
            const response = await axios.post(`/get-all-org-data`, {org_id: this.state.orgId});

            if (response.data && response.data !== 'error') {
                this.setState({org: response.data});
            }
        })();
        // return({orgName: "OrgName",
        //         adminName: "adminName",
        //         field: "fieldOfActivity",
        //         description: "orgDescription",
        //         orgPhone:"!!!!!!!!!!!!!orgPhone",
        //         orgEmail: 'orgEmail',
        //         country: 'country',
        //         city: "city",
        //         street:'street',
        //         building:'building',
        //         pc:'postal code',
        //         minDonation: 'minDonation'
        //         })
    }

    handleSubmit = (e) => {

    alert("TODO: Enter updated details to DB")
    e.preventDefault();
    }

    handleChange(event) {
        const { name, value, type, checked } = event.target
        type === "file" ? this.setState({ [name]: event.target.files[0] }) : this.setState({ [name]: value })
    }



    // function 

    render() {
        return(
            <div>
                <Grid container stackable verticalAlign='middle'>
                <Grid.Row>
                    <Grid.Column width={8}>
                        <div>
                <Segment style={{ padding: '3em 0em' }} vertical>
                    <Grid container stackable verticalAlign='middle'>
                        <br></br>
                        <div as='h3' style={{ fontSize: '2em' }}>
                        <Icon size='big' name='handshake' />
                             Nice to work with you.
                        </div>
                        <Label as='a' color='teal' ribbon left>
                            Edit Your Organization Page!
                        </Label>
                        <Segment>
                            <p style={{ fontSize: '1.33em' }}>
                                We have kept your initial information. We invite you to Edit the information bellow:
                            </p>
                        </Segment>
                        <br></br>
                        <br></br>
                        {/* <Grid relaxed='very' stackable centered> */}
                        <Form onSubmit={this.handleSubmit.bind(this)} success>
                            <Divider
                                as='h4'
                                className='header'
                                horizontal
                                style={{ margin: '3em 0em', textTransform: 'uppercase' }}
                            >
                                <a href='#'>
                                    <Icon size='big' name='edit' />
                                    Edit the Details Bellow:
                                </a>
                            </Divider>
                            {/* info about organization */}
                            <h4></h4>
                            
                            <Form.Field>
                                <Form.Input
                                    label='name of organization:'
                                    value={this.state.org && this.state.org.org_name}
                                    iconPosition='left'
                                    placeholder={this.state.org && this.state.org.orgName}
                                    name="orgName"
                                    onChange={this.handleChange.bind(this)}
                                />
                            </Form.Field>
                            {/* <br /><br /> */}
                            <Form.Field>
                                {/* <Form.label>name of Admin:</Form.label> */}
                                <Form.Input
                                    label='name of Admin:'
                                    icon='user'
                                    iconPosition='left'
                                    placeholder={this.state.org && this.state.org.admin_name}
                                    value={this.state.org && this.state.org.admin_name}
                                    name="admin_name"
                                    onChange={this.handleChange.bind(this)}
                                />
                            </Form.Field>
                            {/* <br /><br /> */}
                            <Form.Field>
                                <Form.TextArea
                                    rows={2}
                                    label='Field of Activity:'
                                    placeholder={this.state.org && this.state.org.field_of_acctivity}
                                    value={this.state.org && this.state.org.field_of_acctivity}
                                    name='field_of_activity'
                                    onChange={this.handleChange.bind(this)} />
                            </Form.Field>
                            <Form.Field>
                                <Form.TextArea
                                    label='Description:'
                                    value={this.state.org && this.state.org.description}
                                    placeholder={this.state.org && this.state.org.description}
                                    name='description'
                                    onChange={this.handleChange.bind(this)} />
                            </Form.Field>
                            
                            <Divider
                                as='h4'
                                className='header'
                                horizontal
                                style={{ margin: '3em 0em', textTransform: 'uppercase' }}
                            >
                                <a href='#'>
                                    <Icon size='big' name='info circle' />
                                    Organization Contact Info:
                                </a>
                            </Divider>
                            <Form.Input
                                label='Phone Number:'
                                icon='phone'
                                iconPosition='left'
                                placeholder={this.state && this.state.adminPhone}
                                name="phone"
                                disabled
                                onChange={this.handleChange.bind(this)}
                            />
                            <Form.Input
                                label='email:'
                                icon='envelope'
                                iconPosition='left'
                                placeholder={this.state.org && this.state.org.org_admin_id}
                                value={this.state.org && this.state.org.org_admin_id}
                                name="email"
                                disabled
                                onChange={this.handleChange.bind(this)}
                            />
                            <Divider
                                as='h4'
                                className='header'
                                horizontal
                                style={{ margin: '3em 0em', textTransform: 'uppercase' }}
                            >
                                <a href='#'>
                                    <Icon size='big' name='map marker alternate' />
                                    Mailing Address
                                </a>
                            </Divider>
                            <Form.Input
                                label='country:'
                                placeholder={this.state.org && this.state.org.country}
                                value={this.state.org && this.state.org.country}
                                name="country"
                                onChange={this.handleChange.bind(this)}
                            />
                            <Form.Input
                                label='city:'
                                placeholder={this.state.org && this.state.org.city}
                                value={this.state.org && this.state.org.city}
                                name="city"
                                onChange={this.handleChange.bind(this)}
                            />
                            <Form.Input
                                label='street:'
                                placeholder={this.state.org && this.state.org.street}
                                value={this.state.org && this.state.org.street}
                                name="street"
                                onChange={this.handleChange.bind(this)}
                            />  
                            <Form.Input
                                label='Building:'
                                placeholder={this.state.org && this.state.org.building}
                                value={this.state.org && this.state.org.building}
                                name="building"
                                onChange={this.handleChange.bind(this)}
                            />                        
                            <Form.Input
                                label='postal code:'
                                placeholder={this.state.org && this.state.org.pc_num}
                                value={this.state.org && this.state.org.pc_num}
                                name="p_code"
                                onChange={this.handleChange.bind(this)}
                            />
                             <Divider
                                as='h4'
                                className='header'
                                horizontal
                                style={{ margin: '3em 0em', textTransform: 'uppercase' }}
                            >
                                <a href='#'>
                                    <Icon size='big' name='money' />
                                    Bank Information
                                </a>
                            </Divider>
                            <Form.Input
                                label='Bank #:'
                                placeholder={this.state.org && this.state.org.bank_num}
                                value={this.state.org && this.state.org.bank_num}
                                name="bank_num"
                                onChange={this.handleChange.bind(this)}
                            />
                            <Form.Input
                                label='Bank Branch #:'
                                placeholder={this.state.org && this.state.org.branch}
                                value={this.state.org && this.state.org.branch}
                                name="bank_branch"
                                onChange={this.handleChange.bind(this)}
                            />
                            <Form.Input
                                label='Account #:'
                                placeholder={this.state.org && this.state.org.account_num}
                                value={this.state.org && this.state.org.account_num}
                                name="account_num"
                                onChange={this.handleChange.bind(this)}
                            />  
                            <Form.Input
                                label='Account Owner:'
                                placeholder={this.state.org && this.state.org.account_owner}
                                value={this.state.org && this.state.org.account_owner}
                                name="account_owner"
                                onChange={this.handleChange.bind(this)}
                            />
                            <Divider
                                as='h4'
                                className='header'
                                horizontal
                                style={{ margin: '3em 0em', textTransform: 'uppercase' }}
                            >
                                <a href='#'>
                                    <Icon size='big' name='image' />
                                    Upload a Photo:
                                </a>
                            </Divider>
                            <Label>The image you upload will be displayed as your organization's image.<br /></Label>
                            <Segment raised>
                            <input label='Choose image' type='file' onChange={this.onSelectFile} />
                            {this.state.selectedImage && <div name="file_preview">
                                <img src={this.state.selectedImage.path} style={{height: '100px', width: '100px', marginTop: '30px'}}/>                                
                           </div>}
                            <br /><br /><Label>If uploading a few images, the last one uploaded will be displayed</Label><br />
                            </Segment>
                            {/* // TODO : STYLE height */}
                            {/* set minimum donation per month */}
                            <div>
                                <Divider
                                    as='h4'
                                    className='header'
                                    horizontal
                                    style={{ margin: '3em 0em', textTransform: 'uppercase' }}
                                >
                                <a href='#'>
                                    <Icon size='big' name='dollar' />
                                    Set minimum monthly doation
                                </a>
                                </Divider>
                               
                                {/* <label > $ </label> */}
                                {/* <Input 
                                    name="minDonation">{this.state.minDonation}
                                     
                                </Input> */}
{/* 
                                <button onClick={this.decrement.bind(this)}>-</button>
                                <button onClick={this.increment.bind(this)}>+</button> */}
                                {/* // TODO: checke why failed */}

                           
                                <Input labelPosition='right' type='text' placeholder={this.state.org && this.state.org.minDoantion}
                                    name="minDonation"
                                    // defaultValue={this.state.minDonation}
                                    value={this.state.minDonation}
                                    onChange={this.handleChange}
                                >
                                    <Label basic>$</Label>
                                    <input />
                                    <Label>.00</Label>
                                </Input>
                            </div>
                            <br></br>
                            <br></br>
                            <Radio toggle value={this.state.org && this.state.org.one_time_donation}
                                label="Allow One-time Donations" />
                            <Divider
                                    as='h4'
                                    className='header'
                                    horizontal
                                    style={{ margin: '3em 0em', textTransform: 'uppercase' }}
                                >
                                <a href='#'>
                                    <Icon size='big' name='users' />
                                    User Status
                                </a>
                                </Divider>

                            <Radio toggle
                                label="User status is based on the sum of money brought to the organiztion in his downline" />
                            <br></br>
                            <br></br>
                            <Label>Default: User status is based only on the number of Doners in his Downline</Label>
                            <br /><br />
                            <br />
                            <Button content='Submit' primary />
                        </Form>
                        {/* </Grid> */}
                    </Grid>
                </Segment>
            </div>
                        {/* ************************************************* */}
                    </Grid.Column>

                    <Grid.Column floated='right' width={6}>
                    </Grid.Column>
                </Grid.Row>
                </Grid>
                    <Grid container stackable verticalAlign='middle'>
                        <Grid.Row>
                            <Grid.Column width={8} floated='middle'>
                            <div as='h3' style={{ fontSize: '2em' }}>
                        <Icon size='big' name='gift' />
                             Add prizes to your organization:
                        </div>
                                <br></br>
                            <Label as='a' color='red' ribbon>
                            Something Special!
                            </Label>
                            <br></br>
                                <Image bordered rounded size='large' src='data:image/jpeg;base64,' />
                                <br></br>
                                <Button primary size='huge' onClick ={() => this.setState(prevState => {
                                    return {
                                            showAddPrize: !prevState.showAddPrize
                                        }})}>
                                    <Icon name='edit' />
                                    Add Prizes
                                    <Icon name='right arrow' />
                                </Button>
                                {this.state.showAddPrize && <AddPrizes />}
                            </Grid.Column>
                        </Grid.Row>
                    </Grid>
            </div>
        )
    }   
}
export default EditOrgPage;